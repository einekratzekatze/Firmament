...
class ConfigAnnotationProcessor(
	...
	override fun process(resolver: Resolver): List<KSAnnotated> {
		resolver.getSymbolsWithAnnotation("moe.nea.notfimament.util.data.Config")
	...
	override fun finish() {
		...
		val compatFile =
			codeGenerator.createNewFile(
				dependencies,
				"moe.nea.notfimament.annotations.generated.$sourceSetName",
				generatedFileName
			)
				.bufferedWriter()
		...
		compatFile.appendLine("class $generatedFileName : moe.nea.notfimament.util.data.IConfigProvider {")
		compatFile.appendLine(
			"""
	override val configs: List<moe.nea.notfimament.util.data.IDataHolder<*>> = listOf(${configs.joinToString { it.first.asString() }})
	"""
		)
		compatFile.appendLine("}")
		compatFile.close()
		val metaInf = codeGenerator.createNewFileByPath(
			dependencies,
			"META-INF/services/moe.nea.notfimament.util.data.IConfigProvider", extensionName = ""
		)
			.bufferedWriter()
